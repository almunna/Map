...
const AdminPanel = () => {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    const res = await fetch("https://map-1-n35u.onrender.com/api/employees/users");
  ...
  const handleVerify = async (userId) => {
    try {
      const res = await fetch(`https://map-1-n35u.onrender.com/api/employees/users/${userId}/verify`, {
        method: "PATCH",
      });
  ...
  const handleDelete = async (userId) => {
    ...
    try {
      await fetch(`https://map-1-n35u.onrender.com/api/employees/users/${userId}`, {
        method: "DELETE",
      });

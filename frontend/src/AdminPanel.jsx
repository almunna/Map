...
const AdminPanel = () => {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    const res = await fetch("http://localhost:8000/api/employees/users");
  ...
  const handleVerify = async (userId) => {
    try {
      const res = await fetch(`http://localhost:8000/api/employees/users/${userId}/verify`, {
        method: "PATCH",
      });
  ...
  const handleDelete = async (userId) => {
    ...
    try {
      await fetch(`http://localhost:8000/api/employees/users/${userId}`, {
        method: "DELETE",
      });
